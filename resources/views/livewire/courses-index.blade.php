        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex">
            ...
            <div class="relative" x-data = "{ abrir:false }">
                ...
                <div class="absolute right-0 w-40 mt-2 py-2 bg-white border rounded shadow-xl" x-show = "abrir" x-on:click.away = "abrir = false">   
                    ...
                    <a class="cursor-pointer transition-colors duration-200 block px-4 py-2 text-normal text-gray-900 rounded hover:bg-green-700 hover:text-white"
                    wire:click = "$set('category_id', {{$category->id}})" x-on:click = "abrir=false">{{$category->name}}</a>
                    @endforeach
            ...
            <div class="relative" x-data = "{ abrir:false }">
                ...
                <div class="absolute right-0 w-40 mt-2 py-2 bg-white border rounded shadow-xl" x-show = "abrir" x-on:click.away = "abrir = false">   
                    ...
                    <a class="cursor-pointer transition-colors duration-200 block px-4 py-2 text-normal text-gray-900 rounded hover:bg-green-700 hover:text-white" 
                    wire:click = "$set('level_id', {{$level->id}})" x-on:click = "abrir=false">{{$level->name}}</a>
                    @endforeach
                </div>
                <!-- // Cuerpo del desplegable -->
            </div>
